...
const ModelConfig = () => {
  ...
  const handleTrain = async () => {
    ...
    try {
      const res = await mlService.train({ k, test_size: testSize, metric });
    ...
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 32 }}>
        ...
        <div>
          ...
          <div className="font-terminal" style={{ fontSize: 10, color: 'var(--clr-text-dim)' }}>kNN MODEL PARAMETER CONFIGURATION</div>
        </div>
      </motion.div>
